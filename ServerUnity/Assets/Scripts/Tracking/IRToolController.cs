...
namespace IRToolTrack
{
    public class IRToolController : MonoBehaviour
    {
        ...
        //    {
        //        var lineRenderer = edgeObjects[i].GetComponent<LineRenderer>();
        //        Vector3 offset = new Vector3(0.04f, -0.007f, 0.05f);
        //        Vector3 position = transform.position;

        //        Vector3 startVertex = position + (transform.rotation * vertices[edges[i, 0]]);
        //        Vector3 endVertex = position + (transform.rotation * vertices[edges[i, 1]]);

        //        lineRenderer.SetPosition(0, startVertex);
        //        lineRenderer.SetPosition(1, endVertex);
        //    }
        //}
        void Start()
        {
        ...
        void Update()
        {
            try
            {
                ...
                //string logStream = irToolTracking.GetLogStream(identifier);
                //positionText.text = logStream;
                //index++;
                ...
                {
                    positionText.text = "Tracking lost...";
                    for (int i = 0; i < transform.childCount; i++)
                    {
                ...
                */
                ...
                //objectToDisplay.transform.position = transform.position;
                //objectToDisplay.transform.rotation = transform.rotation;
                //UpdateEdges();
            }
            catch (Exception e)
            {
                Debug.Log("Error in Update: " + e.Message);
                positionText.text = "Error in Update: " + e.Message;
            }
        }
    }
}